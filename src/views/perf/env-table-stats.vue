 ...
 * 环境数据量信息（基础配置 → 元数据管理）
 *
 * 原「实体元数据」页拆分出来的库容量部分：表统计列表 / 数据库概览 / 同步表统计。
 * 实体清单一侧在「实体元数据」菜单（/base-config/metadata/entity-meta，走 /metadata/entity-meta/*）。
 */
...
defineOptions({ name: 'env-table-stats' })
...
// ── Tab 切换 ──────────────────────────────────
const activeTab = ref<'stats' | 'dbsizes'>('stats')
  ...
  <div class="env-table-stats">
    ...
    <a-card :bordered="false" class="m-b-8px top-bar">
      ...
      <div class="filter-row">
        ...
        <a-button v-if="activeTab === 'stats'" type="primary" :disabled="!sourceEnvId" @click="handleStatsSearch()">
